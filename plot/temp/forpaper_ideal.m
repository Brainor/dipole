...
% h=draw_plot({etas,[imag(gr(omegapd_index,:));real(gr(omegapd_index,:))]},'$$\frac{\omega^\star_p}{\omega_d}=1.67$$','\eta','\omega/\omega_d','Box','on');
% line(h,'XData',baseline*ones(size(ix)),'YData',h.YLim,'LineStyle','--');
% h.XTick=sort([baseline,h.XTick]);h.XTickLabel{3}=baseline_text;h.TickLabelInterpreter='latex';
% legend('Growth rate','Frequency','Box','off','Location','best')
...
subplot(1,2,2)
draw_plot({0:59,[phim1(1:60);pem1(1:60);phim2(1:60);pem2(1:60)]},'','m','','YScale','log','Box','on');
lgd=legend('$$\Phi(x=1)$$','$$G(x=1)$$','$$\Phi(x=0.7)$$','$$G(x=0.7)$$');lgd.Interpreter='latex';lgd.Box='off';
