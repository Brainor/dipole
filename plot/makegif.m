close all;
%参数调整
filename= 'plot.gif'; %你的gif文件的名字
gift1=0.5;gift=0.5;%t1:第一张图暂停的时间，t其他时间间隔，用来控制播放速度，单位秒
...
for i = 1:length(Selection)
    ...
    if k==1
        imwrite(I,map,filename,'gif','LoopCount',Inf,...
            'DelayTime',gift1);%loopcount只是在i==1的时候才有用
    else
        imwrite(I,map,filename,'gif','WriteMode','append',...
            'DelayTime',gift);%DelaylayTime用于设置gif文件的播放快慢
    end
end
close all